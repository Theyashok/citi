...
if pg_isready -q; then
    ...
    if netstat -an 2>/dev/null | grep -q '\*.5432'; then
        PG_OK=true
...
if [ "$PG_OK" = false ]; then
    ...
    for i in {1..10}; do
        if pg_isready -q && netstat -an 2>/dev/null | grep -q '\*.5432'; then
...
if ! command -v mongod &> /dev/null; then
    echo -e "WARNING: MongoDB (mongod) is not installed - skipping for now"
    echo "Install: apt-get install mongodb (Linux) or brew tap mongodb/brew && brew install mongodb-community (Mac)"
    echo "Note: MongoDB is optional for initial development"
    # exit 1  # Temporarily commented out
fi

# Skip MongoDB checks if not installed
if ! command -v mongod &> /dev/null; then
    echo -e "  \u26a0 MongoDB not available - skipping MongoDB setup"
else
    ...
    if [ "$MONGODB_OK" = true ]; then
        ...
        if [[ "$(uname)" == "Darwin" ]]; then
            # macOS
            MONGODB_SERVICE=$(brew services list 2>/dev/null | awk '/^mongodb/ {print $1}' | head -1)
            if [ -n "$MONGODB_SERVICE" ]; then
                brew services start "$MONGODB_SERVICE" || { echo -e "  \u26a0 Failed to start MongoDB"; exit 1; }
            else
                # Try to start manually
                mkdir -p /usr/local/var/mongodb
                mkdir -p /usr/local/var/log/mongodb
                mongod --dbpath /usr/local/var/mongodb --logpath /usr/local/var/log/mongodb/mongo.log --fork || { echo -e "  \u26a0 Failed to start MongoDB manually"; exit 1; }
            fi
        else
            # Linux
            sudo systemctl start mongod || { echo -e "  \u26a0 Failed to start MongoDB"; exit 1; }
        fi

        # Wait for MongoDB to be ready
        for i in {1..10}; do
            if mongosh --quiet --eval "db.adminCommand({ping:1}).ok" > /dev/null 2>&1; then
                echo -e "  \u2713 MongoDB started and bound to 0.0.0.0:27017"
                break
            fi
            if [ "$i" -eq 10 ]; then
                echo -e "  \u26a0 MongoDB failed to start within 10 seconds"
                exit 1
            fi
            sleep 1
        done
...
if ! command -v docker &> /dev/null; then
    echo -e "WARNING: Docker is not installed - skipping LocalStack for now"
    echo "  LocalStack requires Docker to run. Install Docker Desktop to use LocalStack."
    echo "  Note: LocalStack is optional for initial development"
    # exit 1  # Temporarily commented out
elif ! docker info > /dev/null 2>&1; then
    echo -e "WARNING: Docker is not running - skipping LocalStack for now"
    echo "  Please start Docker Desktop to use LocalStack"
    echo "  Note: LocalStack is optional for initial development"
    # exit 1  # Temporarily commented out
else
